...
原 `split learning mode and management mode after login` 中的“登录后默认进入学习模式”依赖这里提供用户身份与设置，但不属于 `P0-a` 的核心交付。该部分并入 `P1-A：统一学习主路径`。

因此 `P0-a` 不负责重排学习页面和管理页面；它只提供后续重排所需的身份、偏好和数据边界。
