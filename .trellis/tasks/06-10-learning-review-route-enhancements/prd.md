...
* 弱项专题复习模式：从 `/weak-items` 进入专门的补强会话，而不是只手动移出。
* 长周期指标完整增强：计划筛选、知识点标签筛选、周/月聚合、完成率、评分分布、AGAIN 比例、弱项新增/消除趋势、掌握度迁移。
* 新词负载自动调节：根据未来负载和积压给出 dailyNewCount 调整建议，必要时暂停新词引入。

P3 候选：

* 单词和知识点联动：词条、例句、语法点和知识卡建立关联，支持从复习中沉淀知识点。
...
* [ ] P1 子任务按独立任务推进时，不把 P2/P3 候选混入 MVP。
* [ ] P1 子任务完成后，回到本母任务复盘 P2 排期。
