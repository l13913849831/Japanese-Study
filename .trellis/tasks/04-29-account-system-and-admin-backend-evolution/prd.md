...
* [x] 能说明当前账号系统已经覆盖了什么、还缺什么。
* [x] 能明确“用户账号域”和“管理员域”的职责边界。
* [x] 能给出 2-3 个可行的演进路线，并说明推荐方案。
* [x] 能拆出一组可继续排期的父任务 / 子任务。
* [x] 能明确第一阶段管理员后台的 MVP 范围。
...
截至 `2026-06-10`，P0-c 第一阶段账号治理主线已完成：

* `P0-c-1` 已完成 session cookie、CSRF、bootstrap 默认关闭、登录失败锁定、认证审计、审计查询和安全告警。
* `P0-c-2` 已完成 `USER` / `ADMIN` 角色基础、`/api/admin/**` 后端访问控制、`/admin` 前端守卫。
* `P0-c-3` 已完成用户列表 / 搜索 / 详情、启用 / 禁用、后台重置密码、资产摘要、审计事件查询和安全告警展示。

后续账号方向如果继续推进，应按新增强项描述，例如设备会话管理、外部身份接入、API token、云端托管备份或 VIP 治理，不应再按第一阶段用户治理后台从零立项。
